...
- [claude-tabletop](https://github.com/cjcsecurity/claude-tabletop) - Two paired Claude Code skills for project-aware technical tabletop exercises across 9 domains (cybersec, prodsec, devops/SRE, privacy/compliance, data/ML, platform, mobile, AI safety, business continuity). `/tabletop-exercise` surveys the actual stack (manifests, IaC, CI/CD) to pick scenarios from a 54-scenario / 38-inject / 28-NPC library, then generates a facilitator runbook plus an interactive single-file HTML runbook with live exercise timer, per-inject reveal overlays, decision-log auto-timestamping, and JSON export. `/tabletop-aar` consumes that JSON (or filled forms) and drafts a structured After-Action Report with severity-tagged action items. 7 compliance frameworks (NIST CSF, SOC 2, PCI, ISO 27001, HIPAA, GDPR, MITRE ATT&CK). [Live demo](https://live-demo-zeta-one.vercel.app). *By [@cjcsecurity](https://github.com/cjcsecurity)*
