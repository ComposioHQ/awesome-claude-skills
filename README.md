...
- [webclaw](https://github.com/0xMassi/webclaw) - Web extraction MCP server with 10 tools (scrape, crawl, extract, summarize, brand, diff, map, batch, search, research). TLS fingerprinting bypasses anti-bot without a browser. 67% fewer tokens than raw HTML. `npx create-webclaw` auto-configures Claude Desktop and Claude Code.
