...
- [lean](https://github.com/civillizard/claude-lean-skill) - Plan tasks with optimal model routing (Haiku/Sonnet/Opus) to minimize token usage. Decomposes work into steps, assigns cheapest capable model per step, adds quality gates between stages, persists plans to file, and reports savings after execution. Includes companion hook for always-on model suggestions. *By [@civillizard](https://github.com/civillizard)*
