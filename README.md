...
- [deep-context](https://github.com/googlarz/deep-context) - Builds verified context packs from folders (≤200 files): per-file notes with mandatory source citations, contradiction detection, independent red-team pass, self-test calibration, and a one-page actionable digest. For legal, financial, medical, and audit corpora where a confidently-wrong answer is expensive. *By [@googlarz](https://github.com/googlarz)*
- [deterministic-workflow-builder](https://github.com/googlarz/deterministic-workflow-builder) - Turns vague agent workflows into auditable packages: typed workflow.json manifest, steps/*.sh, approval gates, machine-checkable artifact contracts, parallel DAG execution, rollback hooks, doctor/repair commands, bounded AI sidecars. For production agent pipelines that cannot fail silently. *By [@googlarz](https://github.com/googlarz)*
...
- [logic-audit](https://github.com/googlarz/logic-audit) - Cross-artifact consistency auditor — finds bugs that live between files: spec says return None, code raises, tests pass against the wrong behavior. Works on code+tests+docs, contracts, financial reports, research data. 6-phase audit with structural verification gates, evidence grading, and differential mode. *By [@googlarz](https://github.com/googlarz)*
...
- [math-skill](https://github.com/googlarz/math-skill) - Rigorous math solver with SymPy verification, matplotlib visualization, and photo input (handwritten problems, whiteboards). Modes: solver, tutor, proof, exam, worked-solution, lesson. Verifies its own answers programmatically. *By [@googlarz](https://github.com/googlarz)*
...
- [betriebsrat](https://github.com/googlarz/betriebsrat) - German labour law advisor grounded in gesetze-im-internet.de — BetrVG, KSchG, AGG, MuSchG. Reasons through procedure, deadlines, and entitlements for employees and works council members. Every answer cites the specific paragraph with a verification link. *By [@googlarz](https://github.com/googlarz)*
...
- [fashion-skill](https://github.com/googlarz/fashion-skill) - Personal AI stylist: body analysis from photos, wardrobe inventory, outfit recommendations with reasoning, pre-purchase checks, in-store mode, week-ahead planning with weather, feedback loop that learns from wear history, Zalando price alerts, Vinted resell listing generator. *By [@googlarz](https://github.com/googlarz)*
...
- [health-skill](https://github.com/googlarz/health-skill) - Persistent health workspace and longevity companion — connects labs, medications, family history, symptoms, and training across sessions. Includes drug interaction checker, pharmacogenomics (23andMe/AncestryDNA), FHIR import, cycle tracking, menopause support, preventive care schedules, cross-domain pattern detection, and caregiver dashboard for whole households. *By [@googlarz](https://github.com/googlarz)*
