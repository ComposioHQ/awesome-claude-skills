...
- [humanizer](https://github.com/adelaidasofia/humanizer) - Strip AI patterns from text and rewrite in the author's actual voice. Builds a statistical fingerprint from the author's writing corpus, applies bilingual EN/ES rule libraries, and includes a `--diff` mode that scores any draft 0-100 against the fingerprint. Forked from [@blader/humanizer](https://github.com/blader/humanizer) with v2.6+ enhancements. *By [@adelaidasofia](https://github.com/adelaidasofia)*
...
- [ingest-youtube](https://github.com/adelaidasofia/ai-brain-starter/tree/main/skills/ingest-youtube) - Ingest YouTube transcripts into a queryable markdown vault. Mirrors the ingest-* connector pattern (Slack, WhatsApp, Notion, Linear, GitHub, Gmail). Pulls via `yt-dlp` with manual-subs > auto-captions > Whisper fallback priority, cleans VTT timing markers into prose, writes one file per video to `External Inputs/YouTube/<channel>/<date>-<title>.md`, and creates capture-seed stubs when transcripts contain trigger keywords like "the lesson is", "playbook", or "anti-pattern". Bilingual EN+ES default. Idempotent. *By [@adelaidasofia](https://github.com/adelaidasofia)*
...
- [ai-brain-starter](https://github.com/adelaidasofia/ai-brain-starter) - Verification harness around an AI agent so memory compounds instead of corrupts. Markdown vault as ground truth, deterministic pre-write hooks, bi-temporal rule lineage, and a session-end cascade. Bundles journaling with an advisory panel, weekly + monthly insight cycles, knowledge graph extraction with community detection, drift detection, and rule-conflict detection. Inspired by Microsoft DELEGATE-52 (arxiv.org/abs/2604.15597) finding that frontier LLMs corrupt 25% of professional content over 20 edits. *By [@adelaidasofia](https://github.com/adelaidasofia)*
