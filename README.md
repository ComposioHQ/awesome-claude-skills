...
- [Marmot](https://marmot.sh) - Shell-native CLI giving agent harnesses (Claude Code, Codex, OpenCode) one command shape for AI, web search, scraping, and data enrichment across many providers. Composes via shell pipes; ships an installable skill via `npx skills add https://github.com/marmot-sh/marmot --skill marmot` or `/plugin install marmot@marmot`. *By [@marmot-sh](https://github.com/marmot-sh)*
