...
- [Ejentum Reasoning Harness](https://github.com/ejentum/ejentum-mcp/tree/main/skills) - Four SKILL.md files (reasoning, code, anti-deception, memory) backed by an MCP server that injects engineered cognitive scaffolds (failure pattern, procedure, suppression vectors, falsification test) before the first token. Routes autonomously when Claude Code recognizes the trigger conditions for each mode. Install with `npx -y ejentum-mcp` plus `EJENTUM_API_KEY` (free tier 100 calls at [ejentum.com/pricing](https://ejentum.com/pricing)). *By [@ejentum](https://github.com/ejentum)*
