...
description: "Automate Ring Central tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Ring Central Automation via Rube MCP

Automate Ring Central operations through Composio's Ring Central toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Ring Central connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `ring_central`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Ring Central operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Ring Central task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `ring_central` |
