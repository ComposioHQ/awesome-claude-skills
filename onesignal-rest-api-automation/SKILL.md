...
description: "Automate Onesignal Rest Api tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Onesignal Rest Api Automation via Rube MCP

Automate Onesignal Rest Api operations through Composio's Onesignal Rest Api toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Onesignal Rest Api connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `onesignal_rest_api`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Onesignal Rest Api operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Onesignal Rest Api task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `onesignal_rest_api` |
