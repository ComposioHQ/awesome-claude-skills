...
description: "Automate Fillout Forms tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Fillout Forms Automation via Rube MCP

Automate Fillout Forms operations through Composio's Fillout Forms toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Fillout Forms connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `fillout_forms`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Fillout Forms operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Fillout Forms task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `fillout_forms` |
