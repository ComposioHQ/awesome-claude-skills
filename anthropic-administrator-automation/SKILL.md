...
description: "Automate Anthropic Administrator tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Anthropic Administrator Automation via Rube MCP

Automate Anthropic Administrator operations through Composio's Anthropic Administrator toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Anthropic Administrator connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `anthropic_administrator`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Anthropic Administrator operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Anthropic Administrator task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `anthropic_administrator` |
