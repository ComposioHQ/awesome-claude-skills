...
description: "Automate Cloudflare Api Key tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Cloudflare Api Key Automation via Rube MCP

Automate Cloudflare Api Key operations through Composio's Cloudflare Api Key toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Cloudflare Api Key connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `cloudflare_api_key`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Cloudflare Api Key operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Cloudflare Api Key task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `cloudflare_api_key` |
