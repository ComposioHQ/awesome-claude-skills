...
description: "Automate Go To Webinar tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Go To Webinar Automation via Rube MCP

Automate Go To Webinar operations through Composio's Go To Webinar toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Go To Webinar connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `go_to_webinar`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Go To Webinar operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Go To Webinar task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `go_to_webinar` |
