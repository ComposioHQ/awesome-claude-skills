...
description: "Automate Tripadvisor Content Api tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---

# Tripadvisor Content Api Automation via Rube MCP

Automate Tripadvisor Content Api operations through Composio's Tripadvisor Content Api toolkit via Rube MCP.

## Prerequisites

- Rube MCP must be connected (RUBE_SEARCH_TOOLS available)
- Active Tripadvisor Content Api connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `tripadvisor_content_api`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Tripadvisor Content Api operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Tripadvisor Content Api task"}]
...
| Connect account | `RUBE_MANAGE_CONNECTIONS` with toolkit `tripadvisor_content_api` |
